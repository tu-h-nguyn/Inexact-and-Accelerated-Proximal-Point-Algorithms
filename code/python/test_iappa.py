...
def test_khop_bang_ket_qua_da_commit(quartic_res) -> None:
    ...
    # Hai loai cot, hai muc chinh xac khac nhau -- va su khac biet nay do CI do
    # duoc chu khong phai suy doan. Chay lai tren runner cua GitHub cho thay
    # `final_objective` khop TUNG BIT (day lap la truy hoi vo huong mot chieu,
    # khong co BLAS), con hai cot do doc thi lech o chu so cuoi:
    #
    #   -0.5207170429964162  ->  -0.5207170429964159
    #   -1.9888595374794573  ->  -1.988859537479457
    #
    # Ly do: polyfit la mot bai toan binh phuong toi thieu giai bang LAPACK, va
    # thu tu cong don trong do PHU THUOC CPU. Doi hoi tung bit o ca hai nhom se
    # do kim; doi hoi rel=1e-6 o ca hai thi qua long -- no bo lot moi troi so
    # that su nho hon 1e-6 trong cac cot von tai lap duoc chinh xac.
    for method, key in (("IAPPA1 q=7/4", "iappa1"), ("IAPPA2 q=7/4", "iappa2")):
        assert df.loc[method, "final_objective"] == quartic_res[key][-1], method

    for method, key in (("IAPPA1 q=7/4", "slope1"), ("IAPPA2 q=7/4", "slope2")):
        assert df.loc[method, "delta_log_slope_500_2000"] == pytest.approx(
            quartic_res[key], rel=1e-12
        ), method
