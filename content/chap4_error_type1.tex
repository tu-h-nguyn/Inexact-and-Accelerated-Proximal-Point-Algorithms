% Luu y: cac nhan thm:4.1 / thm:4.3 dat theo so hieu trong BAI BAO GOC
% (Salzo-Villa 2012, Dinh ly 4.1 va 4.3). So hieu in ra trong bao cao nay
% khac (3.1 va 3.3) vi bo dem theorem duoc danh theo chuong.
...
\begin{theorem}\label{thm:4.1}
    ...
    \begin{align*}
        t&=(1-\alpha)x+\alpha\nu\\
        \hat x&\approx_1\prox_{\lambda F}(t)\quad\text{với $\eps$-chính xác}\\
        \hat \varphi&=U(z,0,\xi,\alpha)\quad\text{với }z=\prox_{\lambda F}(t),\ \xi=\dfrac{t-z}{\lambda}\in\partial F(\hat x)\\
        \hat A&=(1-\alpha)A\\
...
\begin{theorem}\label{thm:4.3}
    ...
    \end{align*}
    Thì tồn tại $\hat\nu\in\Hil$, $\norm{\hat\nu-\hat u}\le\hat\eta$ và $\hat\varphi_*\in\R$ sao cho nếu $\hat\varphi=\hat\varphi_*+\tfrac{\hat A}2\norm{\cdot-\hat\nu}^2$, được
    \[
    \hat\varphi-F\le(1-\alpha)(\varphi-F)
    \]\[
    \hat\delta+\hat\varphi_*\ge F(\hat x)
    \]
    Cụ thể hơn, hàm $\hat\varphi$ thu được bởi $\hat\varphi=U(t,0,\xi,\alpha)\varphi$ với $t=(1-\alpha)x+\alpha\nu$ và $\xi=\dfrac{t-\prox_{\lambda F}(t)}{\lambda}$
\end{theorem}
\begin{essayonly}
\begin{proof}
...
\begin{equation*}
    ...
    \delta_{k+1}&=(1-\alpha_k)\delta_k+\frac{(\alpha_k\eta_{k+1})^2}{2\lambda_k}
...
\begin{align*}
    ...
    \delta_k &= \frac{A\beta_k}{2} \sum_{i=0}^{k-1} \eta_{i+1}^2, \qquad \eta_{i+1} = \sum_{j=0}^{i} \frac{\varepsilon_j}{\alpha_j}\autotag{eq:eta-delta-closed}
\end{align*}
Công thức \eqref{eq:eta-delta-closed} là mấu chốt để dẫn ra tốc độ hội tụ cho thuật toán \eqref{IAPPA1}.
\end{remark}
\begin{theorem}
...
\begin{proof}
    ...
    \begin{align*}
        ...
        &=\frac{c^2}{(1-p)^2}\left(\frac{(k+1)^{3-2p}}{3-2p}-2p\frac{(k+1)^{2-p}}{2-p}+p^k-\frac{4p^2-7p+2}{(3-2p)(2-p)}\right)\autotag{eq:sum-eta-poly}
    \end{align*}
    Với $p=1$:
    \begin{align*}
        \sum_{i=0}^{k-1}\eta_{i+1}^2&\le c^2\sum_{i=1}^{k}(1+\log\,i)^2\\
        &\le c^2\left(1+\int_{2}^{k+2}(1+\log t)^2dt\right)\\
        &=c^2\left(k+(k+1)\log^2(k+1)-2\log^22\right)\autotag{eq:sum-eta-log}
    \end{align*}
    Kết hợp \eqref{eq:sum-eta-poly} và \eqref{eq:sum-eta-log}, thu được:
