Chứng minh việc sử dụng sai số loại 2 giúp khôi phục tốc độ hội tụ của phiên bản thuật toán điểm cận biên tăng tốc chính xác tuy tương tự phần trước, điểm khác biệt nằm ở công thức cập nhật của đại lượng $\delta_k$.
\begin{theorem}
    ...
    \begin{align*}
        y&=(1-\alpha)x+\alpha\nu\\
        \hat x&\approx_2\prox{\lambda F}(y)\quad\text{với $\eps$-chính xác}\\
    ...
    \end{align*}
    thu được $\hat\delta+\hat\varphi_*\ge F(\hat x)+\dfrac{c}{2\lambda}\norm{y-\hat x}^2\ge F(\hat x)$ với $c=\left(\dfrac{2-\alpha^2}{\hat A\lambda}\right)\ge0$.
\end{theorem}
\begin{essayonly}
\begin{proof}
...
\begin{equation*}
    ...
    \delta_{k+1}&=(1-\alpha_k)\delta+\frac{\eps_k^2}{2\lambda_k}
\end{aligned}\right.\label{IAPPA2}\tag{IAPPA2}
\end{equation*}
\begin{remark}
Định lý trên đưa ra đánh giá dựa trên tham số \(c_k\), từ đó suy ra nếu \(c_k \ge 2-a>0\) và \(\lambda_k\) bị chặn trên thì \(\|y_{k-1}-x_k\| \to 0\).
