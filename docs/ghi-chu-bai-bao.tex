...
Trong \eqref{eq:simple}, số hạng $(1-a_k)\tfrac{t_k}{t_{k+1}}(y_k-x_{k+1})$ chỉ triệt tiêu khi $a_k=1$. Do đó, đặt $a_k=1$, $\lambda_k=\lambda$ (hằng số) và dùng các bước kề chính xác sẽ khôi phục lại thuật toán \textbf{FISTA} của Beck \& Teboulle (2009); còn $a_k=2$ cho lại thuật toán của \textbf{G\"{u}ler} (1992), trong đó số hạng thứ ba vẫn còn.
